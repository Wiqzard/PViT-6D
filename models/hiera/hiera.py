...
class HieraRaw(nn.Module):
    def __init__(
        ...
        #self.norm = norm_layer(embed_dim)
        #self.head = Head(embed_dim, num_classes, dropout_rate=head_dropout)
        #self.pool = nn.AdaptiveAvgPool1d(9)
        #self.pose_head = PoseRegressionHead(768 *9 )
    ...
    def load_my_state_dict(self, state_dict):
        ...
        #rot, trans = self.pose_head(x)
        #return rot, trans
     #  # return x.view(x.shape[0], -1,7,7)
        ##x = x.mean(dim=1)

        #if mask is None:  # x : (B, inp_size/16, inp_size/16, 768)
            #x = x.mean(dim=1)
            #x = self.norm(x)
            #x = self.head(x)
