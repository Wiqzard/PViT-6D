...
def cal_rel_pos_type(
    ...
    """
    sp_idx = 2 if has_cls_token else 0
    q_h, q_w = q_size
    k_h, k_w = k_size
...
class MultiScaleVit(nn.Module):
    ...
    def __init__(
        ...
        if True:
            ...
            # self.cls_embed = nn.Embedding(num_classes, embed_dim)
            # self.bbox_embed = nn.Linear(4, embed_dim)
            # self.rot_token = nn.Linear(2 * embed_dim, embed_dim)
            # self.trans_token = nn.Linear(2 * embed_dim, embed_dim)
            # self.centroid_token = nn.Linear(2 * embed_dim, embed_dim)
            # self.z_token = nn.Linear(2* embed_dim, embed_dim)
            # self.rot_head = MLP(embed_dim, 256, 6, num_layers=3)
            # self.centroid_head = MLP(embed_dim, 256, 2, num_layers=3)
            # self.z_head = MLP(embed_dim, 256, 1, num_layers=3)
    ...
    def set_pose_mode(
        self, cls_embed_dim, num_classes, bbox_embed_dim, late_fusion=False, pred_cls=False
   ):
        del self.head
        self.head = None
        del self.cls_token
        ...
        if pred_cls is not None:
            self.cls_token = nn.Embedding(num_classes, cls_embed_dim)
            self.num_prefix_tokens = 1
            #pos_embed_dim = num_classes + 1
        ...
        if late_fusion:
            ...
            self.rot_token = nn.Linear(in_dim, embed_dim)
            self.centroid_token = nn.Linear(in_dim, embed_dim)
            self.z_token = nn.Linear(in_dim, embed_dim)
        ...
        self.apply(self._pose_init)

    def _pose_init(self):
        common_tokens = ["rot_head", "centroid_head", "z_head"]
        late_fusion_tokens = ["rot_token", "trans_token"]
        early_fusion_tokens = ["cls_embed", "rot_token", "centroid_token", "z_token"]
        for token in common_tokens:
            trunc_normal_tf_(getattr(self, token), std=0.02)
        target_tokens = late_fusion_tokens if self.late_fusion else early_fusion_tokens
        for token in target_tokens:
            trunc_normal_tf_(getattr(self, token), std=0.02)
            nn.init.constant_(getattr(self, token).bias, 0.0)
        trunc_normal_tf_(self.bbox_embed, std=0.02)

    def forward_features(self, x):
        feats, feat_size = self.patch_embed(x["roi_img"])
        B, N, C = feats.shape
        if True:
            if self.late_fusion:
                rot_token = self.rot_token(x["roi_cls"]).unsqueeze(1)
                trans_token = self.trans_token(x["roi_cls"]).unsqueeze(1)
            else:
                cls_token = self.cls_embed(x["roi_cls"])
                bbox_token = self.bbox_embed(x["bbox"])
                combined_token = torch.cat([cls_token, bbox_token], dim=1)
                rot_token = self.rot_token(combined_token).unsqueeze(1)
                trans_token = self.trans_token(combined_token).unsqueeze(1)
            feats = torch.cat([rot_token, trans_token, feats], dim=1)
    ...
    def forward_head_pose(self, feats, x):
        if self.pool:
            feats = feats[:, self.num_prefix_tokens:].mean(1)
        else:
