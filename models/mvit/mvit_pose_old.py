...
def reshape_pre_pool(
    x, feat_size: List[int], num_token: int
) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
    H, W = feat_size
    if num_token:
        cls_tok, x = x[:, :, :num_token, :], x[:, :, num_token:, :]
    else:
        cls_tok = None
...
def cal_rel_pos_type_linear(
    ...
    """
    sp_idx = num_tokens  # 2 if has_cls_token else 0
    q_h, q_w = q_size
    k_h, k_w = k_size
...
#class FeatureWeightedConv2d(nn.Module):
    #def __init__(self, num_features, dim_conv, kernel_size, stride, padding, groups):
        #super(FeatureWeightedConv2d, self).__init__()

        #self.in_channels = dim_conv
        #self.groups = groups
        ...
        #self.linear = nn.Linear(
            #num_features, self.out_channels * 1 * self.kernel_height * self.kernel_width
        #)
    ...
    #def forward(self, x, feature_vector):
        ...
        #batch_size = x.size(0)
        #conv_weights = self.linear(feature_vector)
        #conv_weights = conv_weights.view(batch_size*self.out_channels, 1, *self.kernel_size)
        
        ## Reshape x to treat the batch dimension as the channel dimension for groups
        #x = x.view(1, batch_size*self.in_channels, x.size(2), x.size(3))
        
        ## Apply convolution with groups set to the batch size
        #out = F.conv2d(x, conv_weights, None, self.stride, self.padding, groups=batch_size*self.out_channels)
        
        ## Reshape output to have separate batch and channel dimensions
...
class FeatureWeightedConv2d(nn.Module):
    def __init__(self, num_features, dim_conv, kernel_size, stride, padding, groups):
        ...
        # The output channels must be divisible by the number of groups
        assert self.out_channels % self.groups == 0
        
        # Linear layer to generate convolution weights from the feature vector.
        self.linear = nn.Linear(num_features, self.out_channels * self.in_channels // self.groups * self.kernel_size[0] * self.kernel_size[1])
    ...
    def forward(self, x, feature_vector=None):
        ...
        # Apply convolution with groups set to the batch size
        out = F.conv2d(x, conv_weights, None, self.stride, self.padding, groups=batch_size*self.in_channels)#self.out_channels)
...
class DynamicConv2d(nn.Module):
    def __init__(self, in_channels, out_channels,stride=1, padding=1, kernel_size=3):
        ...
        self.stride = stride
        self.padding = padding
        # We want to generate filters of shape (out_channels, in_channels, kernel_size, kernel_size)
        self.fc = nn.Linear(in_channels, out_channels * in_channels * kernel_size * kernel_size)
        # printfc number parameters
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=kernel_size//2,)

    def forward(self, feature_maps, feature_vector):
        # Generate dynamic filters from the feature vector
        #if feature_vector.ndim > 2:
        #    feature_vector = feature_vector.mean(dim=1).squeeze(1)

        feature_vector = feature_vector.reshape(-1, self.in_channels)
        dynamic_filters = self.fc(feature_vector)
        batch_size, in_channels, H, W = feature_maps.size()
        # Reshape to (batch_size*out_channels, in_channels, kernel_size, kernel_size)
        dynamic_filters = dynamic_filters.view(-1, self.in_channels, self.kernel_size, self.kernel_size)
        
        # Reshape feature_maps to (1, batch_size*in_channels, H, W) for group conv
        feature_maps = feature_maps.view(1, -1, H, W)
        
        # Apply dynamic convolution as a group convolution
        output = F.conv2d(feature_maps, dynamic_filters, groups=batch_size, padding=self.padding, stride=self.stride)
        out_h, out_w = output.shape[-2:]
        
        # Reshape back to (batch_size, out_channels, output_H, output_W)
...
class CrossAttentionConv(nn.Module):
    ...
    def forward(self, feature_map, feature_vector):
        bs, c, h, w = feature_map.shape
        feature_vector = feature_vector.reshape(-1, c)
        
        # Project the feature vector to serve as queries, and the feature map to serve as keys
        query =feature_vector.unsqueeze(2).unsqueeze(3)  # (bs, c, 1, 1)
        #query = self.query_proj(feature_vector).unsqueeze(2).unsqueeze(3)  # (bs, c, 1, 1)
        key = self.key_proj(feature_map)  # (bs, c, h, w)
        
        # Calculate the compatibility score
        compatibility_score = torch.sum(query * key, dim=1, keepdim=True)  # (bs, 1, h, w)
        
        # Scale scores and apply softmax
        attention_weights = F.softmax(compatibility_score * self.scale, dim=-1)  # (bs, 1, h, w)
        attended_features = feature_map * attention_weights  # (bs, c, h, w)
        # Apply convolution to the attended features
        output = self.conv2d(attended_features)  # The shape will depend on kernel_size, padding, and stride
...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        if mode in ("avg", "max"):
            ...
            print(kernel_q, stride_q, padding_q)
            if kernel_q:
                # self.pool_q = nn.Conv2d(
                self.pool_q = FeatureWeightedConv2d(
                    dim_conv,
                    dim_conv,
                    kernel_q,
                    stride=stride_q,
            ...
            if kernel_kv:
                self.pool_k = FeatureWeightedConv2d(
                    dim_conv,
                    dim_conv,
                    kernel_kv,
                    stride=stride_kv,
                ...
                self.pool_v = FeatureWeightedConv2d(
                    dim_conv,
                    dim_conv,
                    kernel_kv,
                    stride=stride_kv,
    ...
    def forward(self, x, feat_size: List[int]):
        ...
        # b = k.permute(0, 1,3,2)
        # a = torch.matmul(q, b)
        # torch.save(a.detach().cpu().half(), "attn.pt")
        if self.pool_q is not None:
            q, q_tok = reshape_pre_pool(q, feat_size, self.num_token)
            q = self.pool_q(q, q_tok[:, :, 2])
        ...
        if self.pool_k is not None:
            k, k_tok = reshape_pre_pool(k, feat_size, self.num_token)
            k = self.pool_k(k, k_tok[:, :, 2])
        ...
        if self.pool_v is not None:
            v, v_tok = reshape_pre_pool(v, feat_size, self.num_token)
            v = self.pool_v(v, v_tok[:, :, 2])
...
class MultiScaleBlock(nn.Module):
    ...
    def _shortcut_pool(self, x, feat_size: List[int]):
        if self.shortcut_pool_attn is None:
            return x
        if self.num_token:
            cls_tok, x = x[:, : self.num_token, :], x[:, self.num_token :, :]
        else:
            cls_tok = None
        B, L, C = x.shape
        H, W = feat_size
...
class MultiScaleVit(nn.Module):
    ...
    def forward_features(self, x, tokens=None):
        feats, feat_size = self.patch_embed(x)
        if tokens is not None:
            feats = torch.cat([tokens, feats], dim=1)
    ...
    # def forward_head(self, feats, x, pre_logits: bool = False):
    # if False:  # self.global_pool:
    ...
    # if self.late_fusion:
    # bbox_embed = self.bbox_embed(x["bbox"])
    ...
    # def forward(self, x):
    # feats, x = self.forward_features(x)
    # x = self.forward_head(feats, x)
    # return x
...
def _create_mvitv2(variant, cfg_variant=None, pretrained=False, **kwargs):
    if kwargs.get("features_only", None):
        raise RuntimeError(
            "features_only not implemented for Multiscale Vision Transformer models."
        )
