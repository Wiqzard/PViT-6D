...
class CrossAttentionConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding, stride):
        ...
        self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, stride=stride, groups=in_channels, bias=False)
...
class MultiScaleAttention(nn.Module):
    def __init__(
        ...
        if mode in ("avg", "max"):
            ...
            if kernel_q:
                self.pool_q = CrossAttentionConv(#nn.Conv2d(
                    dim_conv,
                    dim_conv,
                    kernel_q,
                    stride=stride_q,
            ...
            if kernel_kv:
                ...
                self.norm_v = norm_layer(dim_conv)
#                self.pool_k = CrossAttentionConv(#nn.Conv2d(
#                    dim_conv,
#                    dim_conv,
#                    kernel_kv,
#                    stride=stride_kv,
...
#                )
#                self.norm_k = norm_layer(dim_conv)
#                self.pool_v = CrossAttentionConv(#nn.Conv2d(
#                    dim_conv,
#                    dim_conv,
#                    kernel_kv,
#                    stride=stride_kv,
...
#                self.norm_v = norm_layer(dim_conv)
    ...
    def forward(self, x, feat_size: List[int]):
        ...
        if self.pool_q is not None:
            ...
            q_u = self.pool_q_u(q)
            q_u, q_u_size = reshape_post_pool(q_u, self.num_heads, q_tok)
            q = self.pool_q(q,q_tok[:,:,-1])
            q, q_size = reshape_post_pool(q, self.num_heads, q_tok)
        else:
            q_size = feat_size
        if self.norm_q is not None:
